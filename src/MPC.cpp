...
const int cte_weight = 300;
const int epsi_weight = 300;
const int v_weight = 1;
const int delta_weight = 100;
const int a_weight = 50;
const int delta_smooth_weight = 120000;
 ...
 public:
	...
	void operator()(ADvector& fg, const ADvector& vars) 
	{
		...
		for (size_t t = 1; t < N; t++) 
		{
			...
			AD<double> psides0 = CppAD::atan(coeffs[1] + 2 * x0 * coeffs[2] + 3 * x0 * coeffs[3] * CppAD::pow(x0, 2));

			fg[1 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);
			fg[1 + y_start + t] = y1 - (y0 + v0 * CppAD::sin(psi0) * dt);
			fg[1 + psi_start + t] = psi1 - (psi0 - v0 / Lf * delta0 * dt); // Applying polarity update for simulator steering
			fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
			fg[1 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
			fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0 * delta0 / Lf * dt); // Applying polarity update for simulator steering
		}
	}
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set lower and upper limits for variables actuators in radians.
